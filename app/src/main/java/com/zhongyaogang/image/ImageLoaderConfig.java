...
public class ImageLoaderConfig {
    ...
    public static DisplayImageOptions initDisplayOptions(boolean isShowDefault) {
        ...
        if (isShowDefault) {
            // 默认显示的图片
            displayImageOptionsBuilder.showStubImage(R.mipmap.yaocai1);
            // 地址为空的默认显示图片
            displayImageOptionsBuilder
                    .showImageForEmptyUri(R.mipmap.yaocai1);
            // 加载失败的显示图片
            displayImageOptionsBuilder.showImageOnFail(R.mipmap.yaocai1);
        }
        // 开启内存缓存
